...
import { SUSHI, XSUSHI,TABOO,xTABOO } from '../../constants'
...
// Rinkeby
    ...
    // const sushiBalance = useTokenBalance(TABOO[ChainId.RINKEBY]?.address ?? '')
    // const xSushiBalance = useTokenBalance(xTABOO?.address ?? '')
  
// matic    
    const sushiBalance = useTokenBalance(
        (chainId === ChainId.MATIC)
        ? 
        TABOO[ChainId.MATIC]?.address ?? ''
        :
        (chainId === ChainId.RINKEBY)
        ? 
        TABOO[ChainId.RINKEBY ]?.address ?? ''
        :
        (chainId === ChainId.ROPSTEN)
        ? 
        TABOO[ChainId.ROPSTEN ]?.address ?? ''
        :
        (chainId === ChainId.MAINNET)
        ? 
        TABOO[ChainId.MAINNET ]?.address ?? ''
        :
        TABOO[ChainId.BSC ]?.address ?? ''
        )
        
    const xSushiBalance = useTokenBalance(
        (chainId === ChainId.MATIC)
        ?
        xTABOO[ChainId.MATIC]?.address ?? ''
        :
        (chainId === ChainId.RINKEBY)
        ?
        xTABOO[ChainId.RINKEBY]?.address ?? ''
        :
        (chainId === ChainId.ROPSTEN)
        ?
        xTABOO[ChainId.ROPSTEN]?.address ?? ''
        :
        (chainId === ChainId.MAINNET)
        ?
        xTABOO[ChainId.MAINNET]?.address ?? ''
        :
        xTABOO[ChainId.BSC]?.address ?? ''
        )
            ...
            <div className="flex flex-col w-full min-h-fitContent">
                <div className="flex mb-6 justify-center">
                    ...
                    <div className="hidden md:flex justify-center align-center w-72 ml-6">
                        <img src={XSushiSign} alt={'xTaboo sign'} />
                    </div>
                </div>
