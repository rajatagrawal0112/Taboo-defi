...
import { BAR_ADDRESS, Token, TokenAmount } from 'quest-taboo-sdk'
...
export default function StakeCard({ sushiBalance, xSushiBalance }: StakeCardProps) {
    ...
    const [approvalState, approve] = useApproveCallback(     
        (chainId === 137)
        ? new TokenAmount(
            new Token(137, '0xbea6aff7067d1db51145039c6484a9b670c705ad', 18, 'xTABOO', ''), //adress need to change
            parsedInput.value.toString()
        )  
        :
        (chainId === 4)
        ? new TokenAmount(
            new Token(4, '0x5c09598Dc96e056B269fDFaB9e7d8B0762aE5BeE', 18, 'xTABOO', ''),
            parsedInput.value.toString()
        )
        :
        (chainId === 3)
        ? new TokenAmount(
            new Token(4, '0x1BE28AdF4ee250CBC9c6c80f9cAaC378085a440F', 18, 'xTABOO', ''),
            parsedInput.value.toString()
        )
        :
        (chainId === 56)
        ? new TokenAmount(
            new Token(56, '0xbea6aff7067d1db51145039c6484a9b670c705ad', 18, 'xTABOO', ''),    //address need to change
            parsedInput.value.toString()
        )
        :
         new TokenAmount(
            new Token(1, '0xbea6aff7067d1db51145039c6484a9b670c705ad', 18, 'xTABOO', ''),     //address need to change
            parsedInput.value.toString()
        ),
        (chainId === 137) ? BAR_ADDRESS[137] :(chainId === 4) ? BAR_ADDRESS[4] :(chainId === 3) ? BAR_ADDRESS[3] :(chainId === 56) ? BAR_ADDRESS[56] : BAR_ADDRESS[1]  
    )
            ...
            <div className="bg-dark-900 w-full max-w-xl pt-2 pb-6 md:pb-9 px-3 md:pt-4 md:px-8 rounded">
                ...
                <div className="flex justify-between items-center w-full mt-6">
                    <p className="text-large md:text-h5 font-bold text-high-emphesis">
                        {activeTab === 0 ? i18n._(t`Stake TABOO`) : i18n._(t`Unstake`)}
                    </p>
                    <div className="border-gradient-r-pink-red-light-brown-dark-pink-red border-transparent border-solid border rounded-3xl px-4 md:px-3.5 py-1.5 md:py-0.5 text-high-emphesis text-xs font-medium md:text-caption md:font-normal">
                        {`1 xTABOO = ${xSushiPerSushi.toFixed(4)} TABOO`}
                    </div>
                </div>
                ...
                <div className="relative h-0 bottom-14 w-full pointer-events-none">
                    ...
                    >
                        <div className="flex">
                            ...
                            >
                                {`${input ? input : '0'} ${activeTab === 0 ? '' : 'x'}TABOO`}
                            </p>
                        </div>
